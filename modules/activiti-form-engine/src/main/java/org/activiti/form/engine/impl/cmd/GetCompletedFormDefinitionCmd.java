...
public class GetCompletedFormDefinitionCmd implements Command<CompletedFormDefinition>, Serializable {
	...
	public void fillVariablesWithFormValues(Map<String, JsonNode> submittedFormFieldMap, List<FormField> allFields) {
		for (FormField field : allFields) {
			...
			variables.put(field.getId(), get(fieldValueNode, FormFieldTypes.DATE.equals(fieldType)));
		}
	}
	...
	protected Map<String, JsonNode> fillPreviousFormValues(SubmittedForm submittedForm, FormEngineConfiguration formEngineConfiguration) {
		...
		if (taskId != null && processInstanceId != null) {
			List<SubmittedForm> submittedForms = formEngineConfiguration.getFormService().createSubmittedFormQuery().processInstanceId(processInstanceId).submittedDateBefore(submittedForm.getSubmittedDate()).orderBySubmittedDate().desc().list();
	...
	private Object get(JsonNode pNode, boolean asDate) {
		if (asDate) {
			String fieldValue = pNode.asText();
			try {
				if (StringUtils.isNotEmpty(fieldValue)) {
					LocalDate dateValue = LocalDate.parse(fieldValue);
					return dateValue.toString("d-M-yyyy");
				}
			}
			catch (Exception e) {
				logger.warn("Error parsing form date value for process instance " + processInstanceId + " and task " + taskId + " with value " + fieldValue, e);
				return fieldValue;
			}
		}
		if (pNode.isArray()) {
			ArrayList<Object> list = new ArrayList<>();
			for (Iterator<JsonNode> i = ((ArrayNode) pNode).iterator(); i.hasNext();) {
				list.add(get(i.next(), asDate));
			}
			return list;
		}
		if (pNode.isObject()) {
			HashMap<String, Object> map = new HashMap<>();
			for (Iterator<Map.Entry<String, JsonNode>> fi = ((ObjectNode) pNode).fields(); fi.hasNext();) {
				Map.Entry<String, JsonNode> f = fi.next();
				map.put(f.getKey(), get(f.getValue(), false));
			}
			return map;
		}
		if (pNode.isBoolean()) {
			return Boolean.valueOf(pNode.asBoolean());
		}
		if (pNode.isLong()) {
			return Long.valueOf(pNode.asLong());
		}
		if (pNode.isDouble()) {
			return Double.valueOf(pNode.asDouble());
		}
		return pNode.asText();
	}
}
